...
namespace NeuralNetworks
{
    public class NeuralNetwork
    {
        ...
        public NeuralNetwork(Topology topology)
        {
            ...
            Layers = new List<Layer>(); // Made the layers
        ...
        private double[,] Normalization(double[,] inputs)
        {
            ...
            for (int column = 0; column < inputs.GetLength(1); column++)
            {
                //Average value of Neuron Signal
                var sum = 0.0;
                ...
                //Standard Quadratic Error of a Neuron
                var error = 0.0;
                for (int row = 0; row < inputs.GetLength(0); row++)
                {
        ...
        private double BackPropagation(double expected, params double[] inputs)
        {
            ...
            var result = difference * difference; //SQR difference
            return result;
        }
        ...
        private void SendSignalsToInputNeurons(params double[] inputSignals)
        {
            for (int i = 0; i < inputSignals.Length; i++)
            {
                ...
                // Sent data to out Input Neurons
                neuron.FeedForward(signal);
            }
        }

        private void CreateOuputLayer()
        {
